...
func (ec *executionContext) __resolve_entities(ctx context.Context, representations []map[string]interface{}) []fedruntime.Entity {
	...
	resolveEntity := func(ctx context.Context, typeName string, rep map[string]interface{}, idx []int, i int) (err error) {
		...
		switch typeName {
		case "NginxConfig":
			resolverName, err := entityResolverNameForNginxConfig(ctx, rep)
			if err != nil {
				return fmt.Errorf(`finding resolver for Entity "NginxConfig": %w`, err)
			}
			switch resolverName {
			...
			case "findNginxConfigByID":
				...
				if err != nil {
					return fmt.Errorf(`unmarshalling param 0 for findNginxConfigByID(): %w`, err)
				}
				entity, err := ec.resolvers.Entity().FindNginxConfigByID(ctx, id0)
				if err != nil {
					return fmt.Errorf(`resolving Entity "NginxConfig": %w`, err)
				}
...
func entityResolverNameForNginxConfig(ctx context.Context, rep map[string]interface{}) (string, error) {
	for {
		var (
		...
		return "findNginxConfigByID", nil
	}
	return "", fmt.Errorf("%w for NginxConfig", ErrTypeNotFound)
}
